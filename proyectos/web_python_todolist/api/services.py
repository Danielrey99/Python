...
def actualizar_usuario(usuario_id, nombre_usuario, contrasenha, rol):
    """Actualiza los detalles de un usuario existente."""
    conn = create_connection()
    if conn:
        try:
            cur = conn.cursor()
            cur.execute("UPDATE usuarios SET nombre_usuario = %s, contrasenha = %s, rol = %s WHERE id = %s", (nombre_usuario, contrasenha, rol, usuario_id))
            conn.commit()
            return True
        except psycopg2.Error as e:
